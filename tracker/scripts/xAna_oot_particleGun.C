...
void xAna_oot_particleGun(std::string fin, int processType=-1, float readoutWindow=3){ 
  ...
  for(int k=0;k<2; k++){
    ...
    for(int i=0;i<15;i++){
      ...
      hdiff3[k][i]=(TH1I*)htof_diff->Clone(Form("hdiff3%d%02i",k,i));
  ...
  for (Long64_t ev = 0; ev < data.GetEntriesFast(); ev++) {
    ...
    for(int i=0; i < nHits; i++){
     ...
     // more accurate way of computation if a matched track is found and 
      ...
      hdiff3[decIndex][subLayerIndex]->Fill(tdiff3);
      for(int k=0; k < nBunches; k++)
          {
