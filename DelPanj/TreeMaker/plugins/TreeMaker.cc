...

{
  ...
  fillTrkSimHitInfo_  = iConfig.getParameter<Bool_t>("fillTrkSimHitInfo_");
  ...
  if( fillJetInfo_)   jetTree_=new jetTree("CA8",tree_,iConfig);
  if( fillTrigInfo_)  patHltTree_ = new patHltTree("hlt_",tree_); 
  if( fillPhotInfo_)  photonTree_ = new photonTree("", tree_, iConfig); 

  if( fillTrkSimHitInfo_) trkSimHitTree_ = new trkSimHitTree("",tree_);
...
TreeMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  if( fillTrkSimHitInfo_) trkSimHitTree_ ->Fill(iEvent, iSetup);
